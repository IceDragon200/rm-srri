...
/*
static VALUE
rb_tb_dump2(VALUE self, VALUE depth)
{
  ...
  VALUE rbDataAry = rb_ary_new();

  for(i = 0; i < size; i++) {
    rb_ary_push(rbDataAry, INT2NUM(data[i]));
  }

  VALUE data_str = rb_funcall(
    rbDataAry, sym_pack, 1, rb_str_new2("L*\0"));

  VALUE dump_ary = rb_ary_new();
...
static VALUE
rb_tb_load2(VALUE klass, VALUE rbStr)
{
  const ID sym_unpack = rb_intern("unpack");

  VALUE dump_ary   = rb_marshal_load(rbStr);
  VALUE header_str = rb_ary_entry(dump_ary, 0);
  VALUE data_str   = rb_ary_entry(dump_ary, 1);

  VALUE header_ary = rb_funcall(
    header_str, sym_unpack, 1, rb_str_new2("L5\0"));

  VALUE data_ary = rb_funcall(
    data_str, sym_unpack, 1, rb_str_new2("L*\0"));

  VALUE rbDim, rbXsize, rbYsize, rbZsize, rbSize;
  rbDim   = rb_ary_entry(header_ary, 0);
  rbXsize = rb_ary_entry(header_ary, 1);
  rbYsize = rb_ary_entry(header_ary, 2);
  rbZsize = rb_ary_entry(header_ary, 3);
  rbSize  = rb_ary_entry(header_ary, 4);

  const int dim = NUM2INT(rbDim);
  const int size = NUM2INT(rbSize);

  VALUE rbArgv[3] = { rbXsize, rbYsize, rbZsize };
  VALUE rbTable = rb_class_new_instance(dim, rbArgv, klass);

  GET_TABLE(rbTable, table_struct);

  int* data = table_struct->data;

  int i;
  for(i = 0; i < size; i++) {
    data[i] = NUM2INT(rb_ary_entry(data_ary, i));
  }
